...
func (r *pgUserRepository) GetUserByIDs(ctx context.Context, userIDs []uuid.UUID) ([]*domain.User, error) {
	if len(userIDs) == 0 {
	...
	err := r.readDB.WithContext(ctx).Table("users").Where("id IN ?", userIDs).Find(&users).Error
