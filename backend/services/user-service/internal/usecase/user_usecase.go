...
func (u *UserUsecase) UploadAvatar(ctx context.Context, userID string, avatarURL string) error {
	...
	if err != nil {
		return errors.New("không thể cập nhật avatar vào db")
	}
...
func (u *UserUsecase) GetUserByIDs(ctx context.Context, userIDs []uuid.UUID) ([]*domain.User, error) {
	if len(userIDs) == 0 {
		return []*domain.User{}, nil 
	}

	return u.repo.GetUserByIDs(ctx, userIDs)
}
