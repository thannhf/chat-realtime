...
func (r *conversationRepository) MarkAsRead(
	...
	result := r.db.WithContext(ctx).
		...
		Where(
			"conversation_id = ? AND user_id = ? AND (last_read_message_id IS NULL OR last_read_message_id < ?)",
			conversationID,
			userID,
			messageID,
		).
...
func (r *conversationRepository) GetConversationReadStatuses(ctx context.Context, conversationID uint64) ([]*domain.ConversationRead, error) {
	var reads []*domain.ConversationRead

	err := r.db.WithContext(ctx).Where("conversation_id = ?", conversationID).Find(&reads).Error
